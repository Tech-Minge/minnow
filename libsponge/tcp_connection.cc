...
void TCPConnection::send_tcpsegment() {
...
void TCPConnection::send_rst() {
    ...
    set_ack_and_win(seg);
    seg.header().rst = true;
    _segments_out.push(seg); // just one
}
...
void TCPConnection::segment_received(const TCPSegment &seg) {
    ...
    if (header.rst) {
        ...
        if (seg.length_in_sequence_space()) {
            ...
            send_tcpsegment(); // true here! peer send message, we must ack it!
        } else {
            send_tcpsegment(); // does here actually `true`?
        }
    }
}
...
void TCPConnection::tick(const size_t ms_since_last_tick) {
    ...
    if (_sender.consecutive_retransmissions() > TCPConfig::MAX_RETX_ATTEMPTS) {
        ...
        send_tcpsegment(); // if timeout, retransmit; false or true?
    }
}
...
void TCPConnection::end_input_stream() {
    ...
    _sender.fill_window();
    send_tcpsegment(); // why ack true here?
}
